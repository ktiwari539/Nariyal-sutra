...
async function settleStorefront(page,width){
...
async function storefront(viewport,label){
  ...
  await settleStorefront(page,viewport.width);
  await page.waitForSelector('#scroll-cinema',{state:'attached',timeout:10000});

  if(viewport.width<=980){
    const reel=page.locator('.mobile-story-reel');
    must(await reel.count()===1,`${label}: mobile cinematic reel missing`);
    const state=await reel.evaluate(el=>{const s=getComputedStyle(el),r=el.getBoundingClientRect();const imgs=[...el.querySelectorAll('img')];return {display:s.display,visibility:s.visibility,w:r.width,h:r.height,broken:imgs.filter(i=>i.complete&&i.naturalWidth===0).map(i=>i.getAttribute('src')||'')};});
    ...
    await reel.screenshot({path:path.join(OUT,`${label}-cinematic.png`)});
    must(!errors.length,`${label}: page errors ${errors.join(' | ')}`);
    console.log(`${label.toUpperCase()} CINEMATIC PASS`,JSON.stringify(state));
    await context.close();return;
  }

  await page.waitForSelector('#v20-story-film',{state:'attached',timeout:10000});
  await page.waitForFunction(()=>document.querySelectorAll('#v20-story-film .v21-real-rain img').length>=6,null,{timeout:10000});
  ...
  for(const sample of samples){
    await page.evaluate(({top,range,p})=>scrollTo(0,Math.round(top+range*p)),{top:metrics.top,range,p:sample.p});
    await page.waitForTimeout(220);
    const state=await page.locator('#v20-story-film').evaluate((el,prop)=>({p:Number.parseFloat(getComputedStyle(el).getPropertyValue('--p'))||0,value:Number.parseFloat(getComputedStyle(el).getPropertyValue(prop))||0}),sample.prop);
    must(state.value>=sample.min,`${label}: ${sample.name} state not visible ${JSON.stringify(state)}`);
    states.push({name:sample.name,...state});
    await page.screenshot({path:path.join(OUT,`${label}-${sample.name}.png`),fullPage:false});
  }
  const rain=await page.evaluate(()=>[...document.querySelectorAll('#v20-story-film .v21-real-rain img')].map(i=>({src:i.getAttribute('src')||'',ok:i.complete&&i.naturalWidth>0})));
...
try{
  ...
  const state=await page.evaluate(()=>({title:document.title,path:location.pathname,sidebar:!!document.querySelector('#apSidebar'),imageTargets:!!document.querySelector('#apV38ImageTargets')}));
  ...
  await page.screenshot({path:path.join(OUT,'admin-business-command-center.png'),fullPage:false});
  await context.close();
