...
function mimeFor(file){
  ...
  return ({'.js':'application/javascript','.css':'text/css','.html':'text/html','.json':'application/json','.svg':'image/svg+xml','.png':'image/png','.jpg':'image/jpeg','.jpeg':'image/jpeg','.webp':'image/webp','.mp4':'video/mp4','.webmanifest':'application/manifest+json'})[ext]||'application/octet-stream';
}
async function installProdAssetMirror(page){
  await page.route(`${PROD_ORIGIN}/assets/**`,async route=>{
    ...
    if(fs.existsSync(file)&&fs.statSync(file).isFile()){
      await route.fulfill({status:200,contentType:mimeFor(file),body:fs.readFileSync(file)});
    }else{
      await route.fulfill({status:404,contentType:'text/plain',body:'QA mirror: missing repository asset'});
    }
  });
}
...
try{
  for(const pageName of pages){
    ...
    page.on('response',r=>{
      ...
      if((sameSite(r.url())||u.origin===PROD_ORIGIN)&&r.status()>=400)local404.push(`${r.status()} ${r.url()}`);
    });
    page.on('pageerror',e=>consoleErrors.push(`pageerror: ${e.message}`));

    const response=await page.goto(`${BASE}/${pageName}`,{waitUntil:'domcontentloaded',timeout:30000}).catch(()=>null);
    ...
    await page.waitForTimeout(pageName==='index.html'?5400:1500);

    const state=await page.evaluate(()=>{
      const body=document.body,html=document.documentElement;
      const brokenImages=[...document.images].filter(i=>i.currentSrc&&i.complete&&i.naturalWidth===0).map(i=>i.currentSrc);
      const visibleBroken=[...document.querySelectorAll('.ns-media-failed')].filter(e=>{const s=getComputedStyle(e),r=e.getBoundingClientRect();return s.display!=='none'&&s.visibility!=='hidden'&&r.width>2&&r.height>2;}).length;
      ...
      return {title:document.title,bodyHeight:Math.max(body?.scrollHeight||0,html.scrollHeight||0),scrollWidth:Math.max(body?.scrollWidth||0,html.scrollWidth||0),viewport:innerWidth,brokenImages,visibleBroken,emptyLarge,runtime:window.NS_V421_STABILIZATION||null,assetRuntime:window.NS_V421_ASSET_STATUS||null};
    });
    ...
    for(const e of [...new Set(local404)].slice(0,12))fail(pageScope,`request failed: ${e}`);

    if(pageName==='index.html'){
      if(!state.runtime)fail(pageScope,'V42.1 stabilization runtime did not initialize');
      const critical=await page.evaluate(()=>({collection:!!document.querySelector('#collection'),order:!!document.querySelector('#order'),contact:!!document.querySelector('#contact'),hero:!!document.querySelector('.hero'),nav:!!document.querySelector('nav'),footer:!!document.querySelector('footer')}));
  ...
  for(const vp of viewports){
    ...
    const metrics=await page.evaluate(()=>{
      ...
      return {scrollWidth:Math.max(html.scrollWidth,body.scrollWidth),viewport:innerWidth,bodyHeight:Math.max(html.scrollHeight,body.scrollHeight),mainVisible:main?visible(main):true,introVisible:intro?visible(intro):false,heroVisible:hero?visible(hero):false,failedVisible:[...document.querySelectorAll('.ns-media-failed')].filter(visible).length,brokenImages:[...document.images].filter(i=>visible(i)&&i.currentSrc&&i.complete&&i.naturalWidth===0).map(i=>i.currentSrc)};
    });
