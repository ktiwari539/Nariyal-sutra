...
try{
 ...
 must(await page.locator('.ap-view[data-view="overview"] > .ap-kpis').evaluate(e=>e.hidden),'Legacy hard-coded KPI row remains visible');
 must(await page.locator('.ap-view[data-view="overview"] > .ap-grid-2').evaluate(e=>e.hidden),'Legacy demo attention/chart grid remains visible');
 ...
 // Review KPI must deep-link to the real Review Queue and create visible dashboard context.
 ...
 // Quick Communication must open the existing authoritative composer, not create a parallel form.
 ...
 // Search should route a known Admin record to its owning workspace.
 ...
 // Business pulse segments must be actionable and apply the existing payment filter when the option exists.
 ...
 // Responsive: command center must stay within viewport without horizontal overflow.
