...
async function inspectMediaLayout(page,label){
 ...
 const result=await page.evaluate(()=>{
  ...
  const rows=cards.map(card=>{
   ...
   const styleSnapshot=e=>{const s=getComputedStyle(e);return {display:s.display,width:s.width,height:s.height,minHeight:s.minHeight,maxHeight:s.maxHeight,aspectRatio:s.aspectRatio,position:s.position,contentVisibility:s.contentVisibility,contain:s.contain,visibility:s.visibility,overflow:s.overflow,gridRow:s.gridRow,flex:s.flex}};
   return {id:card.dataset.id||'',card:{left:cr.left,top:cr.top,right:cr.right,bottom:cr.bottom,width:cr.width,height:cr.height},visual:vr&&{top:vr.top,bottom:vr.bottom,height:vr.height},body:br&&{top:br.top,bottom:br.bottom,height:br.height},buttons,nw,nh,withinViewport,computed:img?{image:styleSnapshot(img),visual:styleSnapshot(visual),card:styleSnapshot(card)}:null,fit:img?getComputedStyle(img).objectFit:null,portrait:nh>nw*1.15,landscape:nw>nh*1.15,imgRect:img?(()=>{const r=img.getBoundingClientRect();return {top:r.top,bottom:r.bottom,height:r.height}})():null};
  });
