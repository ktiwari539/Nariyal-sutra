...
try{
...

 {
  ...
  const set=await context.newPage();await set.goto(`${BASE}/admin-set-password.html?token=invalid-${Date.now()}`,{waitUntil:'domcontentloaded'});await set.waitForTimeout(20);if(!/invalid|expired|new invitation/i.test((await set.locator('body').innerText()).toLowerCase()))fail('admin:auth:set-password','invalid setup token has no safe error');await set.close();
 }
 await context.close();
