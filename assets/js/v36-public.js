(function(){
  ...
  function installImageFallback(img){
    if(!img||img.dataset.nsMediaGuard==='1')return;img.dataset.nsMediaGuard='1';
    const original=img.currentSrc||img.getAttribute('src')||'';if(original)img.dataset.nsOriginalSrc=original;normalizeMediaUrl(img,'src');
    if(img.hasAttribute('srcset'))img.setAttribute('srcset',(img.getAttribute('srcset')||'').split(',').map(part=>{const bits=part.trim().split(/\s+/);if(bits[0]&&isDeployScoped(bits[0])){const local=sameOriginCandidate(bits[0]);if(local)bits[0]=local;}return bits.join(' ');}).join(', '));
    let retriedLocal=false;const recover=()=>{const current=img.currentSrc||img.getAttribute('src')||'';if(!retriedLocal&&isDeployScoped(current)){retriedLocal=true;const local=sameOriginCandidate(current);if(local&&local!==current){img.src=local;return;}}img.classList.add('ns-media-failed');img.dataset.nsMediaFailure='unrecovered';};
    img.addEventListener('error',recover);if(img.complete&&img.naturalWidth===0)setTimeout(recover,0);
  }
  function installVideoFallback(video){
    if(!video||video.dataset.nsMediaGuard==='1')return;video.dataset.nsMediaGuard='1';normalizeMediaUrl(video,'poster');video.querySelectorAll('source').forEach(s=>normalizeMediaUrl(s,'src'));
    const fallback=()=>{video.classList.add('ns-video-failed');video.dataset.nsMediaFailure='unrecovered';try{video.pause?.();}catch(_){}};
    video.addEventListener('error',fallback,{once:true});video.querySelectorAll('source').forEach(s=>s.addEventListener('error',fallback,{once:true}));if(video.error)setTimeout(fallback,0);
  }
  ...
  function installExperienceCorrections(){
    ...
    const style=document.createElement('style');style.id='ns-v421-user-corrections';
    style.textContent=`
      ...
      #jungle-intro.ns-intro-owned .ji-logo,#jungle-intro.ns-intro-owned .ji-eyebrow,#jungle-intro.ns-intro-owned .ji-tagline{visibility:visible!important;opacity:1!important}
  ...
  const introOwner={started:false,finished:false,timer:0,enforcer:0,reduced:false};
  function holdIntro(intro,skip,main){
    ...
    intro.style.setProperty('display','block','important');intro.style.setProperty('visibility','visible','important');intro.style.setProperty('opacity','1','important');intro.style.setProperty('pointer-events','auto','important');
  ...
  function enforceCinematicRunway(){
    const film=document.getElementById('v20-story-film');if(!film)return;film.dataset.v36FastHarvest='0';
    if(innerWidth>980){film.dataset.desktopRunway='390vh';film.style.setProperty('display','block','important');film.style.setProperty('position','relative','important');film.style.setProperty('height','390vh','important');film.style.setProperty('min-height','390vh','important');}
  ...
  function enforceCinematicViewport(){
    const film=document.getElementById('v20-story-film');if(!film||innerWidth<=980)return;const stage=film.querySelector('.v20-story-sticky');if(!stage)return;
    const rect=film.getBoundingClientRect(),active=rect.top<=0&&rect.bottom>=innerHeight;
    stage.style.setProperty('left','0','important');stage.style.setProperty('right','0','important');stage.style.setProperty('height','100vh','important');stage.style.setProperty('min-height','680px','important');stage.style.setProperty('overflow','hidden','important');stage.style.setProperty('isolation','isolate','important');stage.style.setProperty('transform','none','important');
    if(active){stage.style.setProperty('position','fixed','important');stage.style.setProperty('top','0','important');stage.style.setProperty('bottom','auto','important');stage.style.setProperty('width','100vw','important');film.dataset.nsViewportState='active';}
    else{stage.style.setProperty('position','absolute','important');stage.style.setProperty('width','100%','important');if(rect.top>0){stage.style.setProperty('top','0','important');stage.style.setProperty('bottom','auto','important');film.dataset.nsViewportState='before';}else{stage.style.setProperty('top','auto','important');stage.style.setProperty('bottom','0','important');film.dataset.nsViewportState='after';}}
    film.dataset.nsLayerContract='v421-visual-recovery';film.dataset.nsViewportContract='v36-fixed-stage';
  }
  ...
  function stabilize(){startIntroOwner();enforceCinematicRunway();enforceCinematicViewport();assignDistinctProductImages();guardMedia();}

  document.documentElement.classList.add('ns-runtime-stabilized');installExperienceCorrections();ensureCinematicOwner();stabilize();
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',stabilize,{once:true});[120,450,1000,2200].forEach(ms=>setTimeout(stabilize,ms));
  let cinematicViewportRaf=0;const requestCinematicViewport=()=>{if(cinematicViewportRaf||innerWidth<=980)return;cinematicViewportRaf=requestAnimationFrame(()=>{cinematicViewportRaf=0;enforceCinematicViewport();});};
  addEventListener('scroll',requestCinematicViewport,{passive:true});addEventListener('resize',()=>{clearTimeout(enforceCinematicRunway.t);enforceCinematicRunway.t=setTimeout(()=>{enforceCinematicRunway();enforceCinematicViewport();},80);},{passive:true});
  ...
  window.NS_V421_STABILIZATION={build:'2026-09-13-single-intro-owner',guardedMedia:document.querySelectorAll('img,video').length,introDisabled:false,introDurationMs:3800,reducedIntroDurationMs:REDUCED_INTRO_MS,introOwner:'v36-single-owner',introAsset:'/assets/images/nariyal-coconut-grove.webp',homepageHeroAsset:'/assets/images/nariyal-premium-hero.webp',desktopRunway:'390vh',cinematicLayerContract:'v421-visual-recovery',cinematicViewportContract:'v36-fixed-stage',viewport:{w:innerWidth,h:innerHeight}};
})();