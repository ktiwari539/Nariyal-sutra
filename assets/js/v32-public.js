...
const MAP={hero:()=>$('#home'),ticker:()=>$('#main-site > .ticker'),collection:()=>$('#collection'),cinematic:()=>$('#scroll-cinema'),'brand-moment':()=>$('#main-site > .brand-moment'),harvest:()=>$('#harvest-film'),'live-motion':()=>$('#live-motion'),benefits:()=>$('#main-site > .benefits'),quality:()=>$('#main-site > .nutrition'),pricing:()=>$('#price-list'),'our-story':()=>$('#our-story'),'how-to-order':()=>$('#how-to-order'),uses:()=>$('#coconut-uses'),'people-teaser':()=>$('#people-of-nariyal'),'people-marquee':()=>$('#nsPeopleMarquee'),faq:()=>$('#faq'),order:()=>$('#order'),contact:()=>$('#contact')};
function isHome(){return /(^|\/)index\.html$|\/$/.test(location.pathname);}
function clearLegacySequenceMarkers(){['v421-cinematic-film','v421-cinematic-film-pro','v20-story-film'].forEach(id=>{const el=document.getElementById(id);if(el?.dataset?.v32Section==='cinematic')delete el.dataset.v32Section;});}
...
let applying=false;
function applySequence(){if(!Store||!isHome()||applying)return;const main=$('#main-site'),footer=$('#main-site > footer');if(!main||!footer)return;clearLegacySequenceMarkers();const s=migrateState();if(!s||!Array.isArray(s.sections))return;applying=true;try{[...s.sections].sort((a,b)=>(+a.order||999)-(+b.order||999)).forEach(sec=>{const node=MAP[sec.id]?.();if(!node)return;node.hidden=sec.visible===false;node.dataset.v32Section=sec.id;if(sec.visible!==false)main.insertBefore(node,footer);});}finally{applying=false;}}
function watchCurrentCinematic(){
 if(!isHome()||!document.body)return;
 const settle=()=>[0,250,800,1600].forEach(ms=>setTimeout(applySequence,ms));
 if(document.getElementById('scroll-cinema')){settle();return;}
 const mo=new MutationObserver(muts=>{for(const m of muts){for(const n of m.addedNodes){const film=n.nodeType===1&&(n.id==='scroll-cinema'?n:n.querySelector?.('#scroll-cinema'));if(film){mo.disconnect();settle();return;}}}});
...
function init(){clearLegacySequenceMarkers();prepareIntroLetters();migrateState();watchCurrentCinematic();[80,360,900,1800,3200,5000].forEach(ms=>setTimeout(applySequence,ms));window.addEventListener('nsv421:change',()=>setTimeout(applySequence,25));window.addEventListener('nsv421:production-ready',()=>setTimeout(applySequence,25));}
