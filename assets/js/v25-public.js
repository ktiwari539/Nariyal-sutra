...
function visibleIds(st,items){
 const width=innerWidth||1200,limit=width<=640?+st.mobileLimit||0:width<=980?+st.tabletLimit||0:+st.desktopLimit||0;
 return limit>0?items.slice(0,Math.max(1,limit)):items;
}
function streamSection(key,st,items){
 ...
 return `<section class="v25-story-stream" id="${esc(anchor)}" data-stream="${esc(key)}" data-stream-mode="${mode}" style="--v25-speed:${+st.speed||48}s"><div class="v25-story-stream-head"><div><small>${key==='ambassadors'?'Ambassadors':key==='promoters'?'Promoters':'Community'}</small><h3>${esc(st.title||key)}</h3></div><p>${esc(st.subtitle||'')}</p></div><div class="v25-story-viewport"><div class="v25-story-track ${reverse?'reverse':''} ${mode==='static'?'is-static':''}"><div class="v25-story-group">${inner}</div>${mode==='loop'?`<div class="v25-story-group" aria-hidden="true">${inner}</div>`:''}</div></div><div class="v25-story-stream-foot">Full-picture treatment · Admin controls image selection, order, speed, direction and device limits</div></section>`;}
function normalizeAfterFailure(root,id){
 const cards=[...root.querySelectorAll(`.v25-story-person[data-id="${CSS.escape(id)}"]`)];cards.forEach(c=>c.remove());
 const groups=[...root.querySelectorAll('.v25-story-group')],live=groups[0]?.querySelectorAll('.v25-story-person').length||0;
 if(live<2){root.querySelector('.v25-story-track')?.classList.add('is-static');root.dataset.streamMode='static';if(groups.length>1)groups[1].remove();}
 if(!live)root.remove();
}
...
function render(){
 if(!Store||!$('#v25PeopleStreams'))return;
 const s=Store.load(),streams=s.peopleStreams||{},host=$('#v25PeopleStreams');let html='';
 for(const key of ['ambassadors','promoters','community']){
   const st=streams[key]||{};if(st.enabled===false)continue;
   const ids=[...new Set(st.selectedIds||[])],items=ids.map(id=>eligible(s,id)).filter(Boolean);
   if(!items.length)continue;html+=streamSection(key,st,items);
 }
 host.innerHTML=html;const world=host.closest('.v25-people-worlds');if(world)world.hidden=!html;if(MediaDB?.hydrate)MediaDB.hydrate(host);wireFallback(host,s);
}
...
function init(){render();addTelegramGateNote();addEventListener('nsv421:change',render);addEventListener('resize',()=>clearTimeout(init.r)||(init.r=setTimeout(()=>{init.r=0;render();},120)),{passive:true});}
