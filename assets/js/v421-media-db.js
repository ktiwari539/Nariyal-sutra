...
function ensureApprovedAmbassadors(s){
 ...
 for(const m of AMBASSADOR_ITEMS){const i=s.media.findIndex(x=>x.id===m.id);if(i>=0)s.media[i]=Object.assign({},s.media[i],m);else s.media.push(JSON.parse(JSON.stringify(m)));}
...
function seedApprovedAmbassadors(){
 ...
 if(localStorage.getItem(MIG)!=='1')Store.audit?.(s,'Approved ambassadors surfaced in Admin and rotating reel','AMB101 / AMB102 / AMB201');
 Store.save(s);localStorage.setItem(MIG,'1');
}
function stabilizeAmbassadorLoads(){
 const Store=window.NSV421Store;if(!Store||Store.__ambassadorLoadStable)return;
 const originalLoad=Store.load.bind(Store);
 Store.load=function(){return ensureApprovedAmbassadors(originalLoad());};
 Store.__ambassadorLoadStable=true;
}
