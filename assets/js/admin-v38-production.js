...
async function init(){targetPanel();renderHomepageSequence();bindSequenceOwnership();bindTargetActions();bindCustomSectionOwnership();bindTaskOwnership();setTimeout(()=>{targetPanel();if(!window.__NS_V421_ADMIN_V40__)renderTargets();renderHomepageSequence();bindSequenceOwnership();},250);setTimeout(()=>{targetPanel();if(!window.__NS_V421_ADMIN_V40__)renderTargets();renderHomepageSequence();bindSequenceOwnership();},900);productionNotice();bindSaveStatus();const bridge=window.NSV421ProductionBridge;if(bridge?.isProduction){const r=await bridge.ready;if(r?.error)return;productionNotice();bindUpload();if(!window.__NS_V421_ADMIN_V40__)renderTargets();setTimeout(bindUpload,800);}window.addEventListener('nsv421:change',()=>{if(!window.__NS_V421_ADMIN_V40__)renderTargets();renderHomepageSequence();setTimeout(renderHomepageSequence,0);if(window.NSV421ProductionBridge?.isProduction)setTimeout(bindUpload,20);});}
