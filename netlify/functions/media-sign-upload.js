...
async function adminRole(claims,token,projectId,ownerUid,ownerEmail){
  const uid=String(claims.sub||''),email=String(claims.email||'');
  if(claims.email_verified!==true||!email)return '';
  if(uid===ownerUid)return email===ownerEmail?'Owner':'';
  const url=`https://firestore.googleapis.com/v1/projects/${encodeURIComponent(projectId)}/databases/(default)/documents/adminRoles/${encodeURIComponent(uid)}`;
  const r=await fetch(url,{headers:{authorization:`Bearer ${token}`}});if(!r.ok)return '';
  const doc=await r.json(),role=String(field(doc,'role','stringValue')||''),status=String(field(doc,'status','stringValue')||''),active=field(doc,'active','booleanValue')===true,roleEmail=String(field(doc,'email','stringValue')||'');
  if(status!=='Active'||active!==true||roleEmail!==email)return '';
  return ['Admin','Manager','Operations','Content','Support','Sales'].includes(role)?role:'';
}
exports.handler = async (event) => {
  ...
  const role=await adminRole(claims,token,projectId,ownerUid,ownerEmail);if(!['Owner','Admin','Manager','Content'].includes(role))return json(403,{error:'This active verified Admin role cannot upload public media.'},origin);
