...
async function adminRole(claims,token,appCheckToken,projectId,ownerUid,ownerEmail){
  ...
  const r=await fetch(url,{headers:firebaseHeaders(token,appCheckToken)});if(!r.ok)return '';
